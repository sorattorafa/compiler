...
def get_var(node_param):
    ...
    if haves_id:
        ...
        node_param_name = node_param_name[:len(node_param_name) - 1]
    return node_param_name     
# funcao para podar a árvore
def simplify_tree(raiz): 
    # percorre a árvore
    for node_iter in LevelOrderIter(raiz): 
        # se o no tiver somente um filho
        if len(node_iter.children) == 1: 
            # se o filho do no tiver algum filho
            if node_iter.children[0].children:
                nod_name = get_name(node_iter) 
                # se o no nao for o no programa
                if nod_name != 'programa': 
                    # filhos
                    children = node_iter.children 
                    # pai
                    parent = node_iter.parent 
                    # o pai do neto recebe o pai do no atual
                    ...
                    for i in range(len(list_children)): 
                        # se o filho for o no atual
                        if list_children[i] == node_iter: 
                            # o filho recebe a lista de filhos na pos 0
                            list_children[i] = children[0] 
                    # preenche novamente        
                    node_iter.parent.children = tuple(list_children) 
                    # chama função novamente até chegar na função do programa
                    simplify_tree(raiz)
                    break          